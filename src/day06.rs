...
/// The organizer explains that it's not really a traditional race - instead, you will get a fixed amount of
...
/// race and also the best distance ever recorded in that race. To guarantee you win the grand prize,
...
/// Holding down the button charges the boat, and releasing the button allows the boat to move. Boats move
/// faster if their button was held longer, but time spent holding the button counts against the total race time.
...
pub fn problem1(input: Vec<String>) -> u64 {
    ...
    (0..times.len())
        .map(|i| solve(times[i], distances[i]))
        .product()
}
...
pub fn problem2(input: Vec<String>) -> u64 {
        ...
        .filter_map(|s| {
            s.split(':').nth(1).map(|s| {
                s.chars()
                    ...
                    .fold(0u64, |acc, c| match c.to_digit(10) {
                        Some(n) => acc * 10 + n as u64,
                        None => acc,
                    })
            })
        })
    ...
    solve(parse[0], parse[1])
}

fn solve(t: u64, d: u64) -> u64 {
    (1..((t + 1) / 2))
        .find(|j| (t - j) * j > d)
        .map(|c| 2 * (((t + 1) / 2) - c) + ((t % 2u64 == 0) as u64))
        .unwrap_or(0)
}
